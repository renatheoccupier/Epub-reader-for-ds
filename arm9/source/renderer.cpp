...
void drawBatteryStatus(scr_id scr, u16 baselineY)
{
	const unsigned batt = pmGetBatteryState();
	unsigned level = PM_BATT_LEVEL(batt);
	if(level > 15u) level = 15u;
	const unsigned percent = (level * 100u + 7u) / 15u;
	const bool charging = (batt & PM_BATT_CHARGING) != 0;

	char buf[8];
	if(charging) sprintf(buf, "%3u%%+", percent);
	else sprintf(buf, "%3u%%", percent);

	const u16 iconBodyWidth = 18;
	const u16 iconBodyHeight = 9;
	const u16 iconCapWidth = 2;
	const u16 paddingRight = 6;
	const u16 bodyX2 = screens::layoutX() - paddingRight - iconCapWidth;
	const u16 bodyX1 = bodyX2 - iconBodyWidth;
	const u16 bodyY1 = screens::layoutY() - buttonFontSize + 2;
	const u16 bodyY2 = bodyY1 + iconBodyHeight;
	const u16 fillWidth = (level * (iconBodyWidth - 4u) + 7u) / 15u;
	const int textWidth = strWidth(eUtf8, buf, 0, 0, buttonFontSize);
	const int textX = bodyX1 - textWidth - 4;

	if(fillWidth)
		fillRect(bodyX1 + 2, bodyY1 + 2, bodyX1 + 1 + fillWidth, bodyY2 - 2, Blend(160), scr);
	rect(bodyX1, bodyY1, bodyX2, bodyY2, scr);
	fillRect(bodyX2 + 1, bodyY1 + 3, bodyX2 + iconCapWidth, bodyY2 - 3, Blend(96), scr);
	printStr(eUtf8, scr, textX, baselineY, buf, 0, 0, buttonFontSize);
}
...
void drawStatusBar(scr_id scr, bool framed)
{
	...
	drawBatteryStatus(scr, baselineY);
}
