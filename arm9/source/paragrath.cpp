...
void Book :: parsePar()
{
	if(pimage == parag.type) {
		const int line_height = settings::font_size + settings::line_gap;
		u16 lines = (parag.image_height + line_height - 1) / line_height;
		if(0 == lines) lines = 1;
		parag.lines.assign(lines, Line());
		return;
	}
	...
	for(u32 start=0, end=0; ; start = end+1){
		...
		Word word = {(int)start, (int)end, width, false};
